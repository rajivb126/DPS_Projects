...
function ViewSchoolnews() {
    ...
    const updateDocument = (updatedData) => {
        const { _id, schoolnews_link_file, ...rest } = updatedData;

        // If there's a file to upload, handle it separately
        if (schoolnews_link_file) {
            const formData = new FormData();
            formData.append("schoolnews_link", schoolnews_link_file);
    ...
    const handleInputChange = (e) => {
        ...
        if (name === "schoolnews_link" && files?.length > 0) {
            // Set the file in modalData to be uploaded later
            setModalData(prevData => ({
                ...prevData,
                schoolnews_link_file: files[0] // Store the file for upload
            }));
        } else {
            setModalData(prevData => ({
                ...prevData,
            ...
            <div className='container-fluid'>
                <div className='row g-3 my-2'>
                    <div className='col-12'>
                        <h3 className='text-center pb-3'>View School News Data</h3>
                        <table className='table table-striped table-bordered'>
                            <thead>
                                <tr>
                                    <th>S. NO.</th>
                                    <th style={{ width: '200px' }}>School News Title</th>
                                    <th>School News Link Path</th>
                                    <th style={{ width: '400px' }}>School News Description</th>
                                    <th>Start Date</th>
                                    <th>End Date</th>
                                    <th>Action</th>
                                </tr>
                            ...
                            <tbody>
                                {data.map((item, index) => (
                                    <tr key={item._id}>
                                        ...
                                        <td>{item.schoolnews_heading}</td>
                                        <td>{item.schoolnews_link}</td>
                                        <td>
                                            {item.schoolnews_description
                                                .split(' ')
                                                .slice(0, 15)
                                                .join(' ')}{item.schoolnews_description.split(' ').length > 15 && '...'}
                                        </td>
                    ...
                    <div className="modal-dialog" role="document">
                        <div className="modal-content">
                            <div className="modal-header">
                                <h5 className="modal-title">{mode === 'view' ? 'View SchoolNews Details' : 'Edit SchoolNews Details'}</h5>
                                <button type="button" className="btn-close" data-bs-dismiss="modal" aria-label="Close" onClick={handleCloseModal}></button>
                            </div>
                            <form onSubmit={handleFormSubmit}>
                                <div className="modal-body">
                                    <div className="form-group mb-3">
                                        <label htmlFor="schoolnews_heading" className="form-label">School News Title:</label>
                                        <input type="text" className="form-control" id="schoolnews_heading" name="schoolnews_heading" value={modalData?.schoolnews_heading || ''} onChange={handleInputChange} />
                                    </div>

                                    <div className="form-group mb-3">
                                        <label htmlFor="schoolnews_link" className="form-label">School News Link Path</label>
                                        {mode === 'view' ? (
                                            <input type="text" className="form-control" id="schoolnews_link" name="schoolnews_link" value={modalData?.schoolnews_link || ''} readOnly />
                                        ) : (
                                            <input type="file" className="form-control" id="schoolnews_link" name="schoolnews_link" onChange={handleInputChange} />
                                        )}
                                    </div>


                                    <div className="form-group mb-3">
                                        <label htmlFor="schoolnews_description" className="form-label">School News Description:</label>
                                        <input type="text" className="form-control" id="schoolnews_description" name="schoolnews_description" value={modalData?.schoolnews_description || ''} onChange={handleInputChange} />
                                    </div>
